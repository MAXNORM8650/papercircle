...
import { GitBranch, TrendingUp, Activity, ChevronDown, ChevronRight, Calendar, Star, Filter } from 'lucide-react';
...
export function LineageGraphTab({ circleId }: LineageGraphTabProps) {
  const { dynamicEdges } = useLineageAnalysis();

  const [databaseEdges, setDatabaseEdges] = useState<Edge[]>([]);
  ...
  // Merge database edges with dynamic edges from context
  const allEdges = useMemo(() => {
    const merged = [...databaseEdges, ...dynamicEdges];

    // Deduplicate based on source + target + type
    const seen = new Set<string>();
    return merged.filter(edge => {
      const key = `${edge.source_paper_id}-${edge.target_paper_id}-${edge.edge_type}`;
  ...
  const loadLineageData = async () => {
    ...
    // Filter sessions by circleId if provided
    ...
    if (circleId) {
      sessionsQuery = sessionsQuery.eq('community_id', circleId);
    }
    ...
    if (edgesData) {
      setDatabaseEdges(edgesData);

      const paperMap = new Map<string, PaperNode>();

      // Build paper map from database edges
      ...
      // Add papers from dynamic edges
      allEdges.forEach((edge) => {
        // For dynamic edges, we might not have full paper info
        // Just create placeholder nodes if needed
        if (edge.source_paper_id && !paperMap.has(edge.source_paper_id)) {
          paperMap.set(edge.source_paper_id, {
            id: edge.source_paper_id,
            title: 'Unknown Paper',
            year: null,
            authors: [],
            abstract: null,
        ...
        if (edge.target_paper_id && !paperMap.has(edge.target_paper_id)) {
          paperMap.set(edge.target_paper_id, {
            id: edge.target_paper_id,
            title: edge.target_title || 'Unknown Paper',
            year: edge.target_year ? parseInt(edge.target_year) : null,
            authors: edge.target_authors ? [edge.target_authors] : [],
            abstract: null,
            venue: null,
            citation_count: 0,
            presented_in_community: false,
      ...
      // Load session counts for community papers
      for (const paperId of communityPaperIds) {
        if (paperMap.has(paperId as string)) {
          ...
          if (circleId) {
            countQuery = countQuery.eq('community_id', circleId);
          }
  ...
  const getRootPapers = (): PaperNode[] => {
    let rootPapers = Array.from(papers.values())
      .filter(paper => paper.parents.length === 0 && paper.children.length > 0);

    // Apply view mode filter
    if (viewMode === 'community') {
      rootPapers = rootPapers.filter(p => p.presented_in_community);
    }

    return rootPapers.sort((a, b) => (b.year || 0) - (a.year || 0));
  };

  // Filter edges based on AI toggle and edge type selection
  const getFilteredEdges = (): Edge[] => {
    return allEdges.filter(edge => {
      // Filter by AI generated status
      if (!showAIEdges && edge.is_ai_generated) {
        return false;
      }

      // Filter by edge type
      if (!selectedEdgeTypes.includes(edge.edge_type)) {
        return false;
      }
  ...
  const renderPaperNode = (paper: PaperNode, depth: number = 0) => {
    ...
    // Filter children based on current filter settings
    const filteredChildren = paper.children.filter(edge => {
      if (!showAIEdges && edge.is_ai_generated) {
        return false;
      }
      if (!selectedEdgeTypes.includes(edge.edge_type)) {
        ...
        >
              ...
              <div className="flex-1">
                ...
                {hasChildren && (
                  <div className="mt-2 flex items-center space-x-2 text-sm text-gray-600">
                    ...
                    <span>{filteredChildren.length} connection{filteredChildren.length > 1 ? 's' : ''}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          {isExpanded && hasChildren && (
            <div className="mt-4 space-y-3 pl-10 border-l-2 border-gray-200">
              {filteredChildren.map((edge) => {
                const childPaper = edge.target_paper_id ? papers.get(edge.target_paper_id) : null;
                if (!childPaper) return null;

                // Determine if this is an AI edge for visual styling
                const isAIEdge = edge.is_ai_generated;
                const edgeBorderClass = isAIEdge ? 'border-2 border-dashed' : 'border';
                ...
                return (
                  <div key={edge.id} className="space-y-3">
                    <div className="flex items-start space-x-3 -ml-10">
                      ...
                      <div className={`flex-1 bg-gray-50 rounded-lg p-3 ${edgeBorderClass} border-gray-200 ${isAIEdge ? 'opacity-70' : ''}`}>
                        <div className="flex items-center space-x-2 mb-2">
                          ...
                          {isAIEdge && (
                            <>
                              <span className="px-2 py-1 bg-blue-50 text-blue-700 rounded text-xs font-medium flex items-center">
                                <span className="mr-1">AI</span>
                                <div className={`w-2 h-2 rounded-full ${getConfidenceBadgeColor(edge.confidence_score)}`}></div>
                              </span>
                            </>
                          )}
                          {edge.verified_by && (
    ...
    <div>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <p className="text-3xl font-bold text-gray-900">{allEdges.length}</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <p className="text-3xl font-bold text-gray-900">
            {allEdges.filter((e) => e.verified_by).length}
          </p>
        </div>
      </div>
      ...
      <div className="mb-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          <button
            onClick={() => setShowAIEdges(!showAIEdges)}
        ...
        {/* Edge Legend */}
        <EdgeLegend
          onFilterChange={(types) => setSelectedEdgeTypes(types)}
          showAIFilter={false}
          aiFilterEnabled={showAIEdges}
          onAIFilterToggle={setShowAIEdges}
        />
      </div>
