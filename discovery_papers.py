...
API_BASE = "http://10.127.30.115:11435"
TEMPERATURE = 0.2
...
AVAILABLE TOOLS:
1. search_arxiv_papers(query, mode, max_results) - Search ArXiv
2. expand_query_concepts(base_query) - Generate query variations
3. rate_paper_relevance(title, abstract, user_query) - Score individual papers
...
2. Search ArXiv using appropriate mode settings
3. Parse results into pandas DataFrame
4. For each paper, call rate_paper_relevance() to get scores
5. Calculate final_score = (w_rel × relevance) + (w_auth × authority) + (w_nov × novelty)
...
2. Search papers using search_arxiv_papers()
