...
-- Fix Session RLS Policies for Circle Privacy
-- Run this in Supabase SQL Editor

-- Drop existing session policies
DROP POLICY IF EXISTS "Users can view sessions" ON sessions;
DROP POLICY IF EXISTS "Circle members can create sessions" ON sessions;
DROP POLICY IF EXISTS "Session creators and community admins can update sessions" ON sessions;

-- Policy 1: View sessions (STRICT - only circle members can see)
CREATE POLICY "Circle members can view sessions"
  ON sessions FOR SELECT
  TO authenticated
  USING (
    -- Sessions without circles are public
    (community_id IS NULL AND visibility = 'public') OR
    -- Session creator can always see
    created_by = auth.uid() OR
    presenter_id = auth.uid() OR
    -- Circle members can see circle sessions
    (
...
-- Policy 2: Create sessions (must be circle member)
CREATE POLICY "Circle members can create sessions"
  ...
  WITH CHECK (
    ...
    (
      -- Sessions without circles are allowed
      community_id IS NULL OR
      -- Must be member of the circle
      EXISTS (
        SELECT 1 FROM community_members
        WHERE community_members.community_id = sessions.community_id
        AND community_members.user_id = auth.uid()
...
-- Policy 3: Update sessions (creator, presenter, or circle admin)
CREATE POLICY "Session owners and circle admins can update"
  ON sessions FOR UPDATE
  TO authenticated
  USING (
    -- Session creator can update
    auth.uid() = created_by OR
    -- Presenter can update
...
-- Policy 4: Delete sessions (only creator or circle admin)
...
-- Also fix community RLS to allow admins to update
DROP POLICY IF EXISTS "Update communities" ON communities;

CREATE POLICY "Circle creator and admins can update"
